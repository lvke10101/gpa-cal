...
CREATE TABLE IF NOT EXISTS "public"."credit_transactions" (
    ...
    CONSTRAINT "credit_transactions_reason_check" CHECK (("reason" = ANY (ARRAY['signup_bonus'::"text", 'unlock_post'::"text", 'comment_post'::"text", 'profile_save'::"text", 'course_add'::"text", 'autosave_batch'::"text", 'course_import'::"text", 'semester_note'::"text", 'admin_grant'::"text", 'purchase'::"text", 'admin_topup'::"text", 'admin_bonus'::"text"])))
);
...
CREATE OR REPLACE FUNCTION "public"."get_credit_history"("p_limit" integer DEFAULT 30, "p_before_created_at" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_before_id" "uuid" DEFAULT NULL::"uuid") RETURNS SETOF "public"."credit_transactions"
...
ALTER FUNCTION "public"."get_credit_history"("p_limit" integer, "p_before_created_at" timestamp with time zone, "p_before_id" "uuid") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."posts" (
    ...
    CONSTRAINT "posts_post_type_check" CHECK (("post_type" = ANY (ARRAY['news'::"text", 'announcement'::"text"])))
);
...
CREATE POLICY "Only contributors can insert posts" ON "public"."posts" FOR INSERT WITH CHECK ((("auth"."uid"() = "author_id") AND (EXISTS ( SELECT 1
   FROM "public"."gpa_data"
  WHERE (("gpa_data"."id" = "auth"."uid"()) AND ("gpa_data"."role" = ANY (ARRAY['contributor'::"text", 'admin'::"text"]))))) AND (("post_type" = 'news'::"text") OR (("post_type" = 'announcement'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."gpa_data"
  WHERE (("gpa_data"."id" = "auth"."uid"()) AND ("gpa_data"."role" = 'admin'::"text"))))))));
...
REVOKE ALL ON FUNCTION "public"."get_credit_history"("p_limit" integer, "p_before_created_at" timestamp with time zone, "p_before_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."get_credit_history"("p_limit" integer, "p_before_created_at" timestamp with time zone, "p_before_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_credit_history"("p_limit" integer, "p_before_created_at" timestamp with time zone, "p_before_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_credit_history"("p_limit" integer, "p_before_created_at" timestamp with time zone, "p_before_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."grant_credits"("p_user_id" "uuid", "p_amount" integer, "p_reason" "text", "p_reference_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."grant_credits"("p_user_id" "uuid", "p_amount" integer, "p_reason" "text", "p_reference_id" "uuid") TO "authenticated";
